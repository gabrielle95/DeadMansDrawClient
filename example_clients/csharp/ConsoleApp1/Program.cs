...
namespace ConsoleApp1
{
    internal class Program
    {
        ...
        //static void Main(string[] args)
        {
            ...
            //var input2 = @"{""eventType"":""ResponseToEffect"",""responseToEffectType"":""Oracle"",""responseToEffectCard"":null}";
            //Console.WriteLine(input2);
            //var x = Newtonsoft.Json.JsonConvert.DeserializeObject<MatchEventDTO>(input2);
            //return;
            ...
            while (true)
            {
                ...
                //var match = await client.GetMatchAsync(matchId, null, null);
                if (match != null)
                {
        ...
        async static Task<MatchDTO?> WaitForActiveMatch()
        {
            ...
            {
                matches = await client.GetMatchesAsync("today", BoolLikeString.True, null, BoolLikeString.True);
                if (matches?.Count == 0) Console.WriteLine("... I am still not the current player in any matchs - retrying query");
            }
        ...
        async static Task PlayAMatch(MatchDTO match)
        {
            ...
            while (isMatchRunning)
            {
                ...
                while (isMatchRunning)
                {
                    ...
                    //        //var typedBody = serializer.Deserialize<T>();
            ...
            //  `winnerIdx:${ ri_matchend.matchEndedWinnerIdx}
            //        scores:${ ri_matchend.matchEndedScores}`
            //);
